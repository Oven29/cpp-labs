...
[[nodiscard]] double CalculateCosWithK(double x, double k) {
    return k * std::cos(x);
}

[[nodiscard]] double CalculateFunction(double x, double k) {
    return x - CalculateCosWithK(x, k);
}

[[nodiscard]] double CalculateSinWithK(double x, double k) {
    return 1 + k * std::sin(x);
}

[[nodiscard]] bool IsValidSegmentSigns(Segment segment, double k) {
    bool isLeftPositive = CalculateFunction(segment.xL, k) > 0;
    bool isRightPositive = CalculateFunction(segment.xR, k) > 0;

    return isLeftPositive != isRightPositive;
}

[[nodiscard]] EquationParameters ReadEquationParametersFromStdin() {
    std::cout << "Введите k (вещественное  число): ";
    double k = 0.;
    std::cin >> k;

    std::cout << "Введите погрешность результата (пример 1e-6): ";
    double epsilon = 0.;
    std::cin >> epsilon;
...
void PrintEquationAnswer(lab::EquationResult equationResult, double epsilon) {
...
EquationResult CalculateIterationMethod(double k, double epsilon, double x0) {
    double prevX = x0;
    double x = CalculateCosWithK(prevX, k);

    int iterationCount = 1;

    while (std::fabs(x - prevX) > epsilon && iterationCount < kMaxIterations) {
        prevX = x;
        x = CalculateCosWithK(x, k);
        ++iterationCount;
    }

    if (iterationCount == kMaxIterations && std::fabs(CalculateFunction(x, k)) > epsilon) {
        return {.solved = false};
    }
...
EquationResult CalculateNewtonMethod(double k, double epsilon, double x0) {
    double prevX = x0;
    double x = prevX - CalculateFunction(prevX, k) / CalculateSinWithK(prevX, k);

    int iterationCount = 1;

    while (std::fabs(x - prevX) > epsilon && iterationCount < kMaxIterations) {
        prevX = x;
        x -= CalculateFunction(x, k) / CalculateSinWithK(x, k);
        ++iterationCount;
    }
